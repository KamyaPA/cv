...
\def\cvprogrammingbox#1#2{
	\begin{tikzpicture}
		\node[draw, fill = green!#20!red, rounded corners](level){#2};
...
\def\cvcontact{
	{\Large\cvvar{contact.title}}\\
	\cvtextmovein\cvvar{contact.email.title}:\cvvar{contact.email.value}\\
	\cvtextmovein\cvvar{contact.phone.title}:\cvvar{contact.phone.value}\\
}

\def\cvlanguages {
	{\Large\cvvar{languages.title}}\\
	\cvlanguageloop{languages.entrances}
}

\def\cvprogramming {
	{\Large\cvvar{programming.title}}\\
...
\begin{tikzpicture}[remember picture, overlay]
	...
	\node[below = of education.south west, anchor = north west, cv text right](experience){\cvinformation};

	%% Draw left 
	\node[anchor = north east, cv text left](picture) at ($(split-north) - (0, \cvmargin)$){
		\begin{center}
		\includegraphics[width=0.618\textwidth]{./img/me.jpg}
		\end{center}
	};
	\node[anchor = north west, cv text left](contact information) at (picture.south west){\cvcontact};
	\node[anchor = north west, cv text left](languages) at (contact information.south west){\cvlanguages};
	\node[anchor = north west, cv text left](programming) at (languages.south west){\cvprogramming};

	%% Draw footer
	\node[anchor = south west, cv text right](experience)
	at ($(split-south) + (0, \cvmargin)$)
